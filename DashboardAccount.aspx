...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type="text/javascript">
        ...
        function viewUserInquiryList(userid, empname) {
            ...
            $('#ifrModuleDashSales').attr('src', pageUrl);
            $find("mpeDashSales").show();
        }

        function showCustomerLedger(id, idname) {
            Sys.UI.DomElement.getElementById('<%= spnModuleHeaderSales.ClientID%>').innerHTML = "Client Ledger";
            var pageUrl = "ClientDetailLedger.aspx?custname=" + idname + "&id=" + id;
            $('#ifrModuleDashSales').attr('src', pageUrl);
            $find("mpeDashSales").show();
        }
        function viewSalesOrder(id) {
            Sys.UI.DomElement.getElementById('<%= spnModuleHeaderSales.ClientID%>').innerHTML = "Sales Order";
            var pageUrl = "SalesOrder.aspx?mode=view&id=" + id;
            $('#ifrModuleDashSales').attr('src', pageUrl);
            $find("mpeDashSales").show();
        }
        function viewSalesTarget(id) {
            Sys.UI.DomElement.getElementById('<%= spnModuleHeaderSales.ClientID%>').innerHTML = "Sales Target";
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div id="divLedger" style="background: linear-gradient(45deg, silver, #f9e3b1); margin-bottom: 10px; padding-left: 10px; padding-right: 10px;">
        ...
        <% if (hdnSerialKey.Value != "PO45-WEFH-RE23-13EX")
            { %>
        <div class="row">
...
<%--                                    <div class="invoice-action-btn">
                    ...
                    <ContentTemplate>
                        <div class="card user-statistics-card cardRound mb-2">
                            <div class="card-content p-0">
                                ...
                                <h5 class="h5Round">
                                    <span id="Span2" runat="server" clientidmode="Static" class="text-top" style="color: white; margin-left: 15px;">Sales Order</span><span id="spnProgressCount" runat="server" clientidmode="Static" class="badge gradient-45deg-yellow-green black-text" style="line-height:25px; float:none;line-height: 25px; vertical-align: text-top; padding: 5px 10px; border-radius: 8px;"></span>
                    ...
                    <ContentTemplate>
                        <div class="card user-statistics-card cardRound mb-2">
                            <div class="card-content p-0">
                                <a class="material-icons background-round gradient-45deg-red-pink white-text float-left p-0 adddialog" data-position="center" data-tooltip="New Sales Order" href="javascript:openDashboardBox('salesorder', 'add', 0);" style="margin: 8px 0 8px 8px;"><i class="material-icons" style="font-size: 20px; padding: 5px;">add</i></a>
                                <h5 class="h5Round">
                                    <span id="Span7" runat="server" clientidmode="Static" class="text-top" style="color: white; margin-left: 15px;">Sales Order</span>
                                    <span id="spnProgressCountPowerEx" runat="server" clientidmode="Static" class="badge gradient-45deg-yellow-green black-text hide" style="line-height:25px; float:none;line-height: 25px; vertical-align: text-top; padding: 5px 10px; border-radius: 8px;"></span>
                                    <asp:DropDownList ID="DropDownList1" runat="server" ClientIDMode="Static" class="myDropDownClass select2-theme browser-default mr-2 float-right hide" AutoPostBack="true" OnSelectedIndexChanged="drpApprovalStatus_SelectedIndexChanged">
                                        <asp:ListItem Text="Pending" Value="Pending" />
                                        <asp:ListItem Text="On Hold" Value="On Hold" />
                                        <asp:ListItem Text="Approved" Value="Approved" />
                                        <asp:ListItem Text="Dispatched" Value="Dispatched" />
                                ...
                                <div style="width: 100%; min-height: 350px; max-height: 350px; padding: 2px 10px 0px 10px; overflow-x: hidden; overflow-y: auto;">
                                    <uc1:myOrderApproval runat="server" ID="myOrderApproval1" />
                                </div>
                            </div>
                        </div>
                    </ContentTemplate>
    ...
    <cc1:ModalPopupExtender ID="myModalPopup" BehaviorID="mpeDashSales" runat="server" ClientIDMode="Static" PopupControlID="Panl1" TargetControlID="Button1" CancelControlID="Button2" BackgroundCssClass="Background">
        ...
        <div id="myModal">
            <div class="modal-header position-sticky gradient-45deg-light-blue-indigo m-0" style="margin-bottom: 10px !important;">
                <h5 class=" z-depth-5" style="font-size: 2rem !important; font-weight: 800; line-height: 50px; margin: 0; text-align: left; padding-left: 15px;">
                    ...
                    <span id="spnModuleHeaderSales" runat="server" clientidmode="static">Manage Module</span>
                    <asp:Button CssClass="btnTopRightCorner float-right" ID="Button2" runat="server" Text="" />
                </h5>
            </div>
        </div>
        <iframe id="ifrModuleDashSales" src="about:blank" runat="server" clientidmode="static" scrolling="auto" style="border: 1px solid silver; background-color: transparent; width: 100%; height: 90%; padding: 5px;"></iframe>
        <br />
    ...
    <script type="text/javascript">
        ...
        function openDashboardBox(view, mode, id) {
            if (view == 'salesorder') {
                Sys.UI.DomElement.getElementById('<%= spnModuleHeaderSales.ClientID%>').innerHTML = "Manage Sales Order";
            ...
            if (view == 'salestarget') {
                Sys.UI.DomElement.getElementById('<%= spnModuleHeaderSales.ClientID%>').innerHTML = "Manage Sales Target";
