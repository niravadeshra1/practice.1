<%@ Page Language="C#" AutoEventWireup="true" MaintainScrollPositionOnPostback="true" ValidateRequest="false" CodeBehind="QuotationSpecification.aspx.cs" Inherits="StarsProject.QuotationSpecification" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        function editItem(id, fldName, element) {
            ...
            // -----------------------------------------
            var link = 'QuotationSpecification.aspx/AutoSaveDetail'
            var myUrl = '{pID:' + id.toString() + ', pFieldName:\'' + fldName + '\', pFieldValue:\'' + ctrlValue + '\',' +
                    ' pQuotationNo:\'' + quotno + '\',' +
                    ' pFinishProductID:\'' + finishprodid + '\' }';
            // -----------------------------------------
            jQuery.ajax({
                type: "POST",
                url: link,
                                    ...
                                    <section class="our-webcoderskull padding-lg" style="height: 320px; overflow-y: auto;">
                                        <div class="container" style="max-width: none; width: 98%; margin: 5px !important;">
                                            <div id="divRemarks" runat="server" clientidmode="Static" class="input-field col m">
                                                <asp:TextBox ID="txtRemarks" runat="server" class="content" ClientIDMode="Static" TabIndex="17" TextMode="MultiLine" MaxLength="500" Style="height: 250px; width:1400px;" />
                                            </div>
                                        </div>
                                    </section>
                                </div>
                                    ...
                                    <section class="our-webcoderskull padding-lg" style="height: 320px; overflow-y: auto;" id="prodsec1">
                                        <div class="container" style="max-width: none; width: 98%; margin: 20px !important;">
                                            <table class="stdtable" cellpadding="0" cellspacing="0" border="0" width="100%">
                                                <asp:Repeater ID="rptProductSpecs" runat="server" ClientIDMode="Static">
                                                    <HeaderTemplate>
                                                        <thead>
                                                            <tr>
                                                    ...
                                                    <ItemTemplate>
                                                        ...
                                                        <tr class="blueShed" id="itemRow<%# Container.ItemIndex + 1 %>">
                                                            <td class="text-center">
                                                                <asp:TextBox class="form-control text-left" Style="width: 50px" ID="newOrder" runat="server" ClientIDMode="Static" TabIndex="<%# (((Container.ItemIndex + 1) * 100) + 1) %>" Text='<%# Eval("itemOrder") %>' />
                                                            </td>
                                                            <td class="text-center">
                                                                <asp:TextBox class="form-control text-left" ID="newGroupHead" runat="server" ClientIDMode="Static" TabIndex="<%# (((Container.ItemIndex + 1) * 100) + 2) %>" Text='<%# Eval("GroupHead") %>' onblur='<%# String.Format("return editItem({0}, \"{1}\", \"{2}\");", Eval("pkID"), "GroupHead", (Container.ItemIndex + 1).ToString()) %>' />
                                                            </td>
                                                            <td class="text-center">
                                                                <asp:TextBox class="form-control text-left" ID="newMaterialHead" runat="server" ClientIDMode="Static" TabIndex="<%# (((Container.ItemIndex + 1) * 100) + 3) %>" Text='<%# Eval("MaterialHead") %>' onblur='<%# String.Format("return editItem({0}, \"{1}\", \"{2}\");", Eval("pkID"), "MaterialHead", (Container.ItemIndex + 1).ToString()) %>' />
                                                            </td>
                                                            <td class="text-center">
                                                                <asp:TextBox class="form-control text-left" ID="newMaterialSpec" runat="server" ClientIDMode="Static" TabIndex="<%# (((Container.ItemIndex + 1) * 100) + 4) %>" Text='<%# Eval("MaterialSpec") %>' onblur='<%# String.Format("return editItem({0}, \"{1}\", \"{2}\");", Eval("pkID"), "MaterialSpec", (Container.ItemIndex + 1).ToString()) %>' />
                                                            </td>
                                                            <td class="text-center">
                                                                <asp:TextBox class="form-control text-left" ID="newMaterialRemarks" runat="server" ClientIDMode="Static" TabIndex="<%# (((Container.ItemIndex + 1) * 100) + 5) %>" Text='<%# Eval("MaterialRemarks") %>' onblur='<%# String.Format("return editItem({0}, \"{1}\", \"{2}\");", Eval("pkID"), "MaterialRemarks", (Container.ItemIndex + 1).ToString()) %>' />
                                                            </td>
                                                        </tr>
                                                    </ItemTemplate>
                                                </asp:Repeater>
