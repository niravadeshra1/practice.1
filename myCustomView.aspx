    ...
    <script type="text/javascript">
        ...
        jQuery(document).ready(function () {
            ...
            if (viewmode == 'inquiry') {
                ...
                //jQuery("#lblCustomHead").text('Complaint Visit List')
                // ---------------------------------------------------------
                dataurl = '/Services/NagrikService.asmx/webCRMSummaryList?pType=' + para1 + "&pUserID=" + para2 + "&pMonth=" + para3 + "&pYear=" + para4;

                colstructure = [
                    { title: "Customer Name", data: "CustomerName", className: "text-center", width: "250" },
                    { title: "Doc. #", data: "DocNo", className: "text-center", width: "90" },
                    { title: "Doc. Date", data: "DocDate", className: "text-center", width: "100", render: function (data, type, full) { return (data) ? moment(data).format('DD-MMM-YYYY') : ''; } },
                    { title: "Basic Amount", data: "BasicAmt", className: "text-center", width: "100" },
                    { title: "Net Amount", data: "NetAmount", className: "text-center", width: "100" },
                    { title: "Initiated By", data: "CreatedByEmployee", className: "text-center", width: "150" },
                ];
            }
            else if (viewmode == 'loghistory') {
                jQuery("#lblCustomHead").text('Log History')
                dataurl = '/Services/NagrikService.asmx/webLogHistory';
                colstructure = [
                    { title: "Module Name", data: "Module Name", className: "text-center", width: "250" },
                    { title: "Activity Name", data: "Activity Name", className: "text-center", width: "90" },
                    { title: "Activity Date", data: "ActivityDate", className: "text-center", width: "100", render: function (data, type, full) { return (data) ? moment(data).format('DD-MMM-YYYY') : ''; } },
                    { title: "Activity Time", data: "ActivityDate", className: "text-center", width: "100", render: function (data, type, full) { return (data) ? moment(data).format('HH:mm') : ''; } },
                    { title: "By User", data: "UserID", className: "text-center", width: "100" },
                    { title: "Employee Name", data: "EmployeeName", className: "text-center", width: "150" },
                ];
            }
            // -------------------------------------------------------------------------
            ...
            if (viewmode == 'inquiry' || viewmode == 'inquirybyuser' ||
                ...
                viewmode == 'userloginlogout' || viewmode == 'dailyactivitybyuser' || viewmode == 'crmsummary') {
                //alert(dataurl)
                jQuery.ajax({
                    url: dataurl,
                    type: "get",
                                ...
                                api.column(groupColumn, { page: 'current' }).data().each(function (group, i) {
                                    if (last !== group) {
                                        if (viewmode == 'dailyactivitybyuser') {
                                            $(rows).eq(i).before('<tr class="group"><td colspan="' + valColspan + '"  style="background-color:#8b8b8f !important; color:white !important; font-size:18px !important; font-weight:bold !important; text-transform:capitalize;"><b>' + groupColumnTitle + '</b>' + moment(group).format('DD-MMM-YYYY') + '</td></tr>');
                                        }
                                        else
                                            $(rows).eq(i).before('<tr class="group"><td colspan="' + valColspan + '"  style="background-color:#8b8b8f !important; color:white !important; font-size:18px !important; font-weight:bold !important; text-transform:capitalize;"><b>' + groupColumnTitle + ' </b>' + group + '</td></tr>');
