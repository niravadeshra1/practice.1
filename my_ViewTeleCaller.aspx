<%@ Page Title="" Language="C#" MasterPageFile="~/StarsSite.Master" AutoEventWireup="true" CodeBehind="my_ViewTeleCaller.aspx.cs" Inherits="StarsProject.my_ViewTeleCaller" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type="text/javascript">
        ...
        function openTimeline(xView, xCustID, xCustName) {
            Sys.UI.DomElement.getElementById('<%= spnModuleHeader.ClientID%>').innerHTML = "Manage Timeline";
        ...
        function showInquiryInfo(inqno) {
            jQuery.ajax({
                ...
                success: function (data) {
                    ...
                    if (returnID > 0) {
                        Sys.UI.DomElement.getElementById('<%= spnModuleHeader.ClientID%>').innerHTML = "Manage TeleCaller Lead";
        ...
        function openContFollowFromTeleCaller(mode, ExtpkID, InqpkID) {
            var pageUrl = "";
            Sys.UI.DomElement.getElementById('<%= spnModuleHeader.ClientID%>').innerHTML = "Manage TeleCaller FollowUp";
            if (InqpkID == '0') {
                Sys.UI.DomElement.getElementById('<%= spnModuleHeader.ClientID%>').innerHTML = "Manage TeleCaller External FollowUp";
                pageUrl = "FollowupExternal.aspx?followupsource=telecaller&mode=" + mode + "&id=0" + "&ExtpkID=" + ExtpkID;
            }
            else {
                Sys.UI.DomElement.getElementById('<%= spnModuleHeader.ClientID%>').innerHTML = "Manage TeleCaller FollowUp";
    ...
    <script type="text/javascript">
        ...
        function openUploadDocument() {
            if (viewmode != null && viewmode != '') {
                var string = viewmode;
                Sys.UI.DomElement.getElementById('<%= spnModuleHeader.ClientID%>').innerHTML = string[0].toUpperCase() + string.slice(1) + " Upload";
        ...
        // -------------------------------------------------------------------------
        let colstructure = [];
        let dataurl, dataurlsearch, para1, para2, para3, viewmode;
        let sortingDef;

        function loadPageLayout() {
            viewmode = jQuery("#hdnView").val();
            // --------------------------------------------------------
            // Section : SI_Master Table
            // --------------------------------------------------------
            //alert('Start Master : ' + viewmode);
            jQuery.ajax({
                url: 'Services/DashboardService.asmx/webGridMaster?modulename=' + viewmode,
                type: "get",
                success: function (data) {
                    var resultdata11 = JSON.parse(data.getElementsByTagName("string")[0].innerHTML);
                    var headerRow = resultdata11[0];
                    // ------------------------------------------------
                    dataurl = headerRow.DataUrl;
                    dataurlsearch = headerRow.DataUrlSearch;
                    jQuery("#hdnTitle").val(headerRow.PageTitle);
                    // ------------------------------------------------

                    if (viewmode == 'telecaller') {
                        para1 = '';
                        para2 = jQuery.trim(jQuery("#drpExternalLeadCat").val());
                        dataurl = dataurl.replace('{0}', para1);
                        dataurl = dataurl.replace('{1}', para2);

                        dataurlsearch = dataurlsearch.replace('{0}', para1);
                        dataurlsearch = dataurlsearch.replace('{1}', para2);
                    }
                    ...
                    // --------------------------------------------------------
                    //alert('Start Child : ' + dataurl)
                    jQuery.ajax({
                        url: 'Services/DashboardService.asmx/webGridDetail?modulename=' + viewmode,
                        type: "get",
                        success: function (data) {
                            debugger;
                            var resultdata11 = JSON.parse(data.getElementsByTagName("string")[0].innerHTML);
                            colstructure = resultdata11;
                            var xUntouchTitle = "Untouch Since <br />Init.By";
                            for (var i = 0; i < colstructure.length; i++) {
                                var col4 = colstructure[i].render;
                                if (col4 != null && col4 != '')
                                    colstructure[i].render = col4.substring(1, col4.length() - 1);
                            }
                            //var oldstructure = [
                            //    {
                            ...
                            //    { title: "City", data: "City", className: "center", width: "100" },
                                ...
                                jQuery.ajax({
                                    ...
                                    success: function (data) {
                                        ...
                                        $('#tblView').DataTable({
                                            ...
                                            deferRender: true, "ordering": false, "bFilter": false,
                                            "bInfo": false, "bPaginate": false, order: sortingDef, "scrollY": 550, "scrollX": true,
...
<%--            document.getElementById('<%=((Label)Master.FindControl("lblModule")).ClientID %>').innerText = jQuery("#hdnTitle").val()
            ...
            //                rtnval += '&nbsp;&nbsp;&nbsp;&nbsp;<a class="tooltipped" data-position="center" data-tooltip="Add Followup" href="javascript:openContFollowFromTeleCaller(\'add\',\'' + row.pkID + '\',\'' + row.InquiryNopkID + '\');"><img src="images/expand.png" alt="" style="width:15px; height:15px;"/></a>';
        ...
        function gridAction(mode, id) {
            ...
            if (mode == 'add' || mode == 'edit' || mode == 'view') {
                Sys.UI.DomElement.getElementById('<%= spnModuleHeader.ClientID%>').innerHTML = "Manage Tele-Caller Leads";
                $('#ifrModule').attr('src', pageUrl);
                $find("mpe").show();
            }
            // -------------------------------------------------------------
